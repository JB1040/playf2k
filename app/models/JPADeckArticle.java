...
public class JPADeckArticle implements DeckArticleRepository  {
    ...
    public CompletionStage<Stream<DeckArticle>> list(int offset,int amount,int tier,DeckArticle.Mode mode, Boolean isStandard) {
        return supplyAsync(() -> wrap(em -> list(em,offset,amount,tier,mode,isStandard)), executionContext);
    }
    ...
    private Stream<DeckArticle> list(EntityManager em,int offset,int amount,int tier,DeckArticle.Mode mode,Boolean isStandard) {
       ...
       return art.subList(offset, Math.min(offset+amount,art.size())).stream();
   }
